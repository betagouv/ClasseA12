"""Migrate the thumbnails from the `videos` records to the thumbnails collection.

For each `videos` record, store the thumbnail data (stored as a data URL in
...
def upload_thumbnails(client, videos):
    for video in videos:
        ...
        # The client points to the "videos" collection but we want to upload to the thumbnails collection.
        attachment_uri = attachment_uri.replace("videos", "thumbnails")

        try:
            print("uploading thumbnail for record id", video["id"])
