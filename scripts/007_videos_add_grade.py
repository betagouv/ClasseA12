"""Add a `grade` field to the `videos` records.

For each `videos` record, add a `grade` field and set it by default to `CP`.
...
$ python 007_videos_add_grade.py --auth "<admin login>:<admin password>" --server "https://<kinto server>/v1/"
...
def main():
    parser = cli_utils.add_parser_options(
        description="Add a `grade` field to the `videos` collection",
